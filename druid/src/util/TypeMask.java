...
public final class TypeMask {
    private Map<SootClass, List<Alloc_Node>> class2allocs = 
        new HashMap<SootClass, List<Alloc_Node>>(1024);
    private List<Alloc_Node> anySubtypeAllocs = new LinkedList<Alloc_Node>();

    public TypeMask( WholeProgPAG pag ) {
        this.pag = pag;
    }
    ...
    final public BitVector get( Type type ) {
        ...
        while(allocNodeListener.hasNext()) {
            Alloc_Node n = allocNodeListener.next();
            for( final Type t : Scene.v().getTypeNumberer()) {
                ...
                if( castNeverFails( n.getType(), t ) ) {
                    ...
                    if( mask == null ) {
                        ...
                        for( final Alloc_Node an : pag.getAllocNodeNumberer()) {
    ...
    protected QueueReader<Alloc_Node> allocNodeListener = null;
    // ** new methods
    ...
    final private BitVector makeMaskOfInterface(SootClass interf) {
        ...
        if (implementers.size() == 0) {
            for (Alloc_Node an : anySubtypeAllocs) ret.set(an.getNumber());
        }
        return ret;
    }
