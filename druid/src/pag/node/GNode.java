...
public class GNode implements ReferenceVariable, Numberable {
    ...
    public void setType( Type type ) {
        if( TypeMask.isUnresolved(type) ) throw new RuntimeException("Unresolved type "+type );
        this.type = type; 
    }
    ...
    public void mergeWith( GNode other ) {
        ...
        if( (other instanceof Var_Node)
                && (myRep instanceof Var_Node )
                && ((Var_Node) other).isInterProcTarget() )
    ...
    protected GNode( WholeProgPAG pag, Type type ) {
        if( TypeMask.isUnresolved(type) ) throw new RuntimeException("Unresolved type "+type );
        this.type = type;
        this.pag = pag;
        replacement = this;
