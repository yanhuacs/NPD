 ...
 */
public class CompressedAPSimplifier extends Rule {

    Set<Predicate> dropPreds = CollectionFactory.newSet();
    @Override
    public void addPredicate(Predicate pred) {

        AccessPath[] aps = pred.getAllAccessPaths();
        for (int i = 0; i < aps.length; ++i) {
            if (aps[i].isCompressed()) {
                dropPreds.add(pred);
            }
            if (aps[i].numOfElemField() >= 3 || aps[i].numOfMapField() >= 3)
                dropPreds.add(pred);
        }
    }
